...
for percent in percents:
    ...
    for (i, path) in enumerate(fhandle.readlines()):
        ...
        enrich=numpy.loadtxt('enrichment/path%s_top%s.0summary.txt'% (i, percent))
        cm=pylab.cm.get_cmap('RdYlBu')
        scatCollection=ax.scatter(data1, data2, data3, linewidth=2, alpha=0.6, c=enrich,vmin=min(enrich), vmax=max(enrich), cmap=cm, s=400)
        fig.canvas.mpl_connect('draw_event',forceUpdate)
        cbar=pylab.colorbar(scatCollection)
        cbar.set_ticks([min(enrich), 1, max(enrich)])
        cbar.set_ticklabels(['Antagonists', 'Equal', 'Agonists'])
        #cbar = fig.colorbar(scatCollection, ticks=[min(enrich), max(enrich)])
        ...
        ax.dist = 10
        pylab.title('Path%s %s%%Flux Ligand Enrichment' % (i, int(stat*100)))
        #pylab.legend(loc=2)
        pylab.savefig('%s/select-%s-path%s-3d-top%s%%chem.png' % (dir, sys, i, percent), dpi=300)
        #pylab.show()

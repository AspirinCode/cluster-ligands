...
def get_matrix(dir, prefix, database, column, max, add=False):
    ...
    for n in range(0, len(database)):
        ...
        indices=-100*numpy.ones((len(database)), dtype=int)
        frames=numpy.where(score==-100)[0]
        count=0
        for (k, name) in enumerate(database):
            rank=numpy.where(names==name.split('-')[1])[0]
            if len(rank)==0:
                print k
                index=frames[count]
                indices[index]=k
...
if __name__ == "__main__":
    ...
    if tanimoto=='pb':
        ...
        add=True
    cutoff=max-cutoff
    database=[x.split('cent/')[1] for x in numpy.loadtxt('%s/eon-dbase.list' % dir, dtype=str)]
